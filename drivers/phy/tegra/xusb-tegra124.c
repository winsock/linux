					  ...
					  unsigned int port)
{
	...
	usb3 = tegra_xusb_find_usb3_port(padctl, port);
	if (!usb3)
		return -ENODEV;

	usb3->context_saved = true;
	lane = usb3->base.lane;
	...
	usb3->tap1 = value & XUSB_PADCTL_IOPHY_MISC_PAD_CTL6_MISC_OUT_TAP_MASK;
	...
	value = padctl_readl(padctl, offset) >>
		XUSB_PADCTL_IOPHY_MISC_PAD_CTL6_MISC_OUT_SHIFT;
	usb3->amp = value & XUSB_PADCTL_IOPHY_MISC_PAD_CTL6_MISC_OUT_AMP_MASK;
...
static const char * const tegra124_utmi_functions[] = {
	"snps",
	"xusb",
	"uart",
};

static const struct tegra_xusb_lane_soc tegra124_utmi_lanes[] = {
	TEGRA124_LANE("utmi-0", 0x004,  0, 0x3, utmi),
	TEGRA124_LANE("utmi-1", 0x004,  2, 0x3, utmi),
	TEGRA124_LANE("utmi-2", 0x004,  4, 0x3, utmi),
};

static struct tegra_xusb_lane *
tegra124_utmi_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
...
static void tegra124_utmi_lane_remove(struct tegra_xusb_lane *lane)
{
...
static const struct tegra_xusb_lane_ops tegra124_utmi_lane_ops = {
	.probe = tegra124_utmi_lane_probe,
	.remove = tegra124_utmi_lane_remove,
};
...
tegra124_utmi_pad_probe(struct tegra_xusb_padctl *padctl,
			...
			struct device_node *np)
{
	...
	mutex_init(&utmi->lock);

	pad = &utmi->base;
	pad->ops = &tegra124_utmi_lane_ops;
	pad->soc = soc;
	...
	err = tegra_xusb_pad_register(pad, &tegra124_utmi_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra124_utmi_pad_remove(struct tegra_xusb_pad *pad)
{
...
static const struct tegra_xusb_pad_ops tegra124_utmi_ops = {
	.probe = tegra124_utmi_pad_probe,
	.remove = tegra124_utmi_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra124_utmi_pad = {
	.name = "utmi",
	.num_lanes = ARRAY_SIZE(tegra124_utmi_lanes),
	.lanes = tegra124_utmi_lanes,
	.ops = &tegra124_utmi_ops,
};
...
static int tegra124_utmi_port_enable(struct tegra_xusb_port *port)
{
	return 0;
}

static void tegra124_utmi_port_disable(struct tegra_xusb_port *port)
{
}

static struct tegra_xusb_lane *
tegra124_utmi_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_find_lane(port->padctl, "utmi", port->index);
}

static const struct tegra_xusb_port_ops tegra124_utmi_port_ops = {
	.enable = tegra124_utmi_port_enable,
	.disable = tegra124_utmi_port_disable,
	.map = tegra124_utmi_port_map,
};
