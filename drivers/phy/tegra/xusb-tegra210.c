...
static void tegra210_hsic_set_idle(struct tegra_xusb_padctl *padctl,
...
static void
tegra210_usb3_set_lfps_detect(struct tegra_xusb_padctl *padctl,
				    ...
				    struct tegra_xusb_mbox_msg *msg)
{
	...
	case MBOX_CMD_STOP_HSIC_IDLE:
		ports = msg->data >> (padctl->soc->ports.hsic.offset + 1);
		resp.data = msg->data;
		resp.cmd = MBOX_CMD_ACK;

		for (i = 0; i < padctl->hsic->soc->num_lanes; i++) {
			if (!(ports & BIT(i)))
				continue;

			if (msg->cmd == MBOX_CMD_STOP_HSIC_IDLE)
				tegra210_hsic_set_idle(padctl, i, false);
			else
				tegra210_hsic_set_idle(padctl, i, true);
		}
		break;
	...
	case MBOX_CMD_ENABLE_SS_LFPS_DETECTION:
		...
		for (i = 0; i < padctl->soc->ports.usb3.count; i++) {
			...
			if (msg->cmd == MBOX_CMD_DISABLE_SS_LFPS_DETECTION)
				tegra210_usb3_set_lfps_detect(padctl, i, false);
			else
				tegra210_usb3_set_lfps_detect(padctl, i, true);
		}
		break;
...
static const char *tegra210_utmi_functions[] = {
	"snps",
	"xusb",
	"uart"
};

static const struct tegra_xusb_lane_soc tegra210_utmi_lanes[] = {
	TEGRA210_LANE("utmi-0", 0x004,  0, 0x3, utmi),
	TEGRA210_LANE("utmi-1", 0x004,  2, 0x3, utmi),
	TEGRA210_LANE("utmi-2", 0x004,  4, 0x3, utmi),
	TEGRA210_LANE("utmi-3", 0x004,  6, 0x3, utmi),
};

static struct tegra_xusb_lane *
tegra210_utmi_lane_probe(struct tegra_xusb_pad *pad, struct device_node *np,
			 unsigned int index)
{
	struct tegra_xusb_utmi_lane *utmi;
	int err;
	...
	INIT_LIST_HEAD(&utmi->base.list);
	utmi->base.soc = &pad->soc->lanes[index];
	utmi->base.index = index;
	utmi->base.pad = pad;
	utmi->base.np = np;

	err = tegra_xusb_lane_parse_dt(&utmi->base, np);
	if (err < 0) {
		kfree(utmi);
		return ERR_PTR(err);
	}
...
static void tegra210_utmi_lane_remove(struct tegra_xusb_lane *lane)
{
	struct tegra_xusb_utmi_lane *utmi = to_utmi_lane(lane);

	kfree(utmi);
}

static const struct tegra_xusb_lane_ops tegra210_utmi_lane_ops = {
	.probe = tegra210_utmi_lane_probe,
	.remove = tegra210_utmi_lane_remove,
};
...
static int tegra210_utmi_phy_power_on(struct phy *phy)
{
	struct tegra_xusb_lane *lane = phy_get_drvdata(phy);
	struct tegra_xusb_utmi_lane *utmi = to_utmi_lane(lane);
	struct tegra_xusb_utmi_pad *pad = to_utmi_pad(lane->pad);
	struct tegra_xusb_padctl *padctl = lane->pad->padctl;
	struct tegra210_xusb_padctl *priv;
	struct tegra_xusb_utmi_port *port;
	unsigned int index = lane->index;
	u32 value;
	int err;

	port = tegra_xusb_find_utmi_port(padctl, index);
	if (!port) {
		dev_err(&phy->dev, "no port found for UTMI lane %u\n", index);
		return -ENODEV;
	}
...
static int tegra210_utmi_phy_power_off(struct phy *phy)
{
	...
	port = tegra_xusb_find_utmi_port(padctl, lane->index);
	if (!port) {
		dev_err(&phy->dev, "no port found for UTMI lane %u\n",
			lane->index);
		return -ENODEV;
...
tegra210_utmi_pad_probe(struct tegra_xusb_padctl *padctl,
			const struct tegra_xusb_pad_soc *soc,
			struct device_node *np)
{
	struct tegra_xusb_utmi_pad *utmi;
	...
	pad = &utmi->base;
	pad->ops = &tegra210_utmi_lane_ops;
	pad->soc = soc;
	...
	utmi->clk = devm_clk_get(&pad->dev, "trk");
	if (IS_ERR(utmi->clk)) {
		err = PTR_ERR(utmi->clk);
		dev_err(&pad->dev, "failed to get trk clock: %d\n", err);
		goto unregister;
	}

	err = tegra_xusb_pad_register(pad, &tegra210_utmi_phy_ops);
	if (err < 0)
		goto unregister;
...
static void tegra210_utmi_pad_remove(struct tegra_xusb_pad *pad)
{
	struct tegra_xusb_utmi_pad *utmi = to_utmi_pad(pad);

	kfree(utmi);
}

static const struct tegra_xusb_pad_ops tegra210_utmi_ops = {
	.probe = tegra210_utmi_pad_probe,
	.remove = tegra210_utmi_pad_remove,
};

static const struct tegra_xusb_pad_soc tegra210_utmi_pad = {
	.name = "utmi",
	.num_lanes = ARRAY_SIZE(tegra210_utmi_lanes),
	.lanes = tegra210_utmi_lanes,
	.ops = &tegra210_utmi_ops,
};
...
static const struct tegra_xusb_lane_soc tegra210_hsic_lanes[] = {
	...
	TEGRA210_LANE("hsic-1", 0x004, 15, 0x1, hsic),
};
...
static int tegra210_utmi_port_enable(struct tegra_xusb_port *port)
{
	return 0;
}

static void tegra210_utmi_port_disable(struct tegra_xusb_port *port)
{
}

static struct tegra_xusb_lane *
tegra210_utmi_port_map(struct tegra_xusb_port *port)
{
	return tegra_xusb_find_lane(port->padctl, "utmi", port->index);
}

static const struct tegra_xusb_port_ops tegra210_utmi_port_ops = {
	.enable = tegra210_utmi_port_enable,
	.disable = tegra210_utmi_port_disable,
	.map = tegra210_utmi_port_map,
};
