...
static int max77620_pinconf_set2(struct pinctrl_dev *pctldev,
		unsigned pin, unsigned long *config, unsigned num_configs)
{
	int i;

	for (i = 0; i < num_configs; i++) {
		int err;
		err = max77620_pinconf_set(pctldev, pin, config[i]);
		if (err < 0)
			return err;
	}
...
static const struct pinconf_ops max77620_pinconf_ops = {
	...
	.pin_config_set = max77620_pinconf_set2,
